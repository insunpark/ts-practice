...
// 디스패치를 위한 타입( Dispatch를 리액트에서 불러올 수 있음), 액션들의 타입을 Dispatc의 Generics로
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'SET_TEXT':
      return {
        ...state,
        text: action.text, // text 자동완성되며, string 타입인걸 알 수 있다.
      };
    case 'SET_COLOR':
      return {
        ...state,
        color: action.color, // color 자동완성되며, color 가 Color 타입인걸 알 수 있다.
      };
    case 'TOGGLE_GOOD':
      return {
        ...state,
...
// SampleStateContext.Provider 와 SampleDispatchContext.Provider 로 children을 감싸서 반환합니다.
